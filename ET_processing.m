...
subplot(1,2,1)
plot(1/Fs:1/Fs:length(PplDiam)/Fs,PplDiam);
ylabel('Raw Pupil diameter (px)')
xlabel('Time (s)')
hold on
...
PplDiam(mis_idx)=nan;
if isnan(PplDiam(1))
    aa=find(isnan(PplDiam)==0);
    PplDiam(1) = PplDiam(aa(1));
end
if isnan(PplDiam(end))
    PplDiam(end) = PplDiam(aa(end));
end
...
subplot(1,2,2)
plot(1/Fs:1/Fs:length(PplDiam)/Fs,PplDiam);
ylabel('(Preprocessed) Pupil diameter (mm)')
xlabel('Time (s)')
...
b_outrng = find(b_d<0.05 | b_d>.7); % Blink duration criteria between 50 and 700 ms
% Eye blink can be as short as 50 ms
% Stern, John A., Larry C. Walrath, and Robert Goldstein. "The endogenous 
% eyeblink." Psychophysiology 21, no. 1 (1984): 22-33.
undef_idx = arrayfun(@colon, b_on(b_outrng), b_off(b_outrng), 'Uniform', false);
...
undef_idx = unique([undef_idx rmv_idx' s_rmv err_smpl]);  % index of undefined samples
...
rmv = [];
% % optional criterion on fixation: on whether it makes one cluster or not
% for j=1:numel(f_on)
%     C_GP_dist = sqrt((EH_gaze_hcoord(f_on(j):f_off(j))-mean(EH_gaze_hcoord(f_on(j):f_off(j)))).^2+(EH_gaze_vcoord(f_on(j):f_off(j))-mean(EH_gaze_vcoord(f_on(j):f_off(j)))).^2);
...
%     end
% end
f_rmv3 = arrayfun(@colon, f_on(rmv), f_off(rmv), 'Uniform', false);
f_rmv3 = cell2mat(f_rmv3);
f_on(rmv)=[];f_off(rmv)=[];f_d(rmv)=[];
f_samples=[];
f_samples = arrayfun(@colon, f_on, f_off, 'Uniform', false);
f_samples = cell2mat(f_samples);
undef_idx = unique([f_rmv f_rmv2 f_rmv3 undef_idx]);
undef_idx(ismember(undef_idx,b_samples))=[];
% -------------- End of fixation detection---------------------------------

% --------------- Visualization of ocular events --------------------------
figure
dtheta(isnan(dtheta))=0;
p1 = plot(1/Fs:1/Fs:length(dtheta)/Fs,dtheta);
hold on
p2 = plot(b_samples/Fs,dtheta(b_samples),'.r');
hold on
p3 = plot(f_samples/Fs,dtheta(f_samples),'.g');
hold on
p4 = plot(s_smpl/Fs,dtheta(s_smpl),'.b');
hold on
p5 = plot(undef_idx/Fs,dtheta(undef_idx),'.c');
legend([p1 p2 p3 p4 p5],{'Gaze velocity', 'blinks','fixations','saccades','Unidentified'})
ylabel('Gaze Velocity (^o/s)')
xlabel('Time (s)')
